"""
This python file contains the implementations for the FALCES-NEW [1] framework.
...
class FALCESNew:
    ...
    def predict(self, test_output, X_pred, y_pred, strategy, metric, weight, sbt, knn_size,
        ...
        if self.pre_processed:
            ...
            pred_nbrs = NearestNeighbors(n_neighbors=10, algorithm='kd_tree').fit(X_pred)

        ###########################################################################################
        #Already predicted values strategy currently only implemented for pre_processed data.######
        ###########################################################################################
        predicted = set()
        #Iterate over every point of the prediction dataset.
        for i in range(len(X_pred)):
            if X_pred.index[i] not in predicted:
                index_list = []

                if self.pre_processed:
                    pred_indices = pred_nbrs.kneighbors(X_pred.iloc[i].values.reshape(1, -1),
                            return_distance=False)
                    pred_index_list = X_pred.index[pred_indices].tolist()[0]
                ...
                #Test all possible model combinations on this dataframe & return the best
                #combination based on given metric and weight.
                if sbt:
                ...
                for pred_index in pred_index_list:
                    if pred_index not in predicted:
                        count = 0
                        for key, item in grouped_df:
                            if isinstance(key, tuple):
    ...
    def efficient_offline(self, model_test, metric, weight, threshold, comb_amount):
        ...
        """
        #Get the globally best model combinations according to the given metric and weight,
        #as well as threshold and comb_amount.
